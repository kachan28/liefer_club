...
func (s SideDish) ToString(conf ExportConfig) string {
	return fmt.Sprintf("%s: Nr. - %s; MwSt. - %d%%; Preis - %s", s.Name, s.Number, s.TaxValue, s.pricesToString(conf))
}

func (s SideDish) pricesToString(conf ExportConfig) string {
	pricesString := ""
